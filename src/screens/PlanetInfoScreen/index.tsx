...
export const PlanetInfoScreen: FC<NavigationStackScreenProps> = (
  ...
  //@ts-ignore
  const { films, residents } = planetsList.find((e) => e.name === planetData.name).planetInfo || {
    films: [],
    residents: [],
  };
      ...
      <TouchableOpacity onPress={() => setHiddenInfo(!hiddenInfo)}>
        {type === 'film' ? (
          <View style={{ backgroundColor: bgColor }}>
            <Text style={{ ...styles.textBoldMd, color: primary }}>
    ...
    return (
      <>
        <View style={{ ...styles.containerHead, backgroundColor: bgColor }}>
          <Icon name={'dribbble'} size={40} color={textColor}>
            {' '}
            {planetData.name}
          </Icon>
        </View>
        <View style={{ backgroundColor: bgColor }}>
          ...
          <Text style={{ ...styles.textBoldMd, color: textColor }}>
